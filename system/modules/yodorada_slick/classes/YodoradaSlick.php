...
class YodoradaSlick extends \Backend
{
    ...
    public function makeSettingArray($confArr, $expArr)
    {
        ...
        return array_merge
            ...
            array_intersect_key
            (
                $this->mapValuesToKeyTypeDefault($confArr), $GLOBALS['YODORADA_SLICK']['settings']
            ),
            $expArr
        );
    }
    ...
    {
        switch ($GLOBALS['YODORADA_SLICK']['settings'][$key]) {
            case "boolean":
    ...
    public function mapValuesToKeyTypeDefault($arr)
    {
        ...
        foreach ($arr as $key => $value) {
            if (!array_key_exists($key, $GLOBALS['YODORADA_SLICK']['settings'])) {
                continue;
            }
